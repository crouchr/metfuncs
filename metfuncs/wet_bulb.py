...
# Python code to calculate wet bulb temperature from dry bulb temperature and dew point using Davies-Jones (2008) method. It was ported from the matlab version by Robert Kopp (https://github.com/bobkopp/bobkopp.github.io/blob/master/software/code/WetBulb.m).
# check the results using : https://www.omnicalculator.com/physics/wet-bulb

# NOTE :https://www.omnicalculator.com/physics/wet-bulb#how-to-calculate-the-wet-bulb-temperature
# talks about Stul formaula that only needs temp and humidity as long a t is witjin certain ranges -
# TODO : Swap this complex formula out for the Stull formaula
...
# Anyway - here is the complicated formula - it works - but is slow...
...
def WetBulb(TemperatureC, Pressure, Humidity, HumidityMode=1):
    ...
    sub = np.where(X <= D)
    wb_temp[sub] = (
                k1[sub] - 1.21 * cold[sub] - 1.45 * hot[sub] - (k2[sub] - 1.21 * cold[sub]) * X[sub] + (0.58 / X[sub]) *
                hot[sub])
    ...
    while (np.max(delta) > 0.01) and (iter <= maxiter):
        es_mb_wb_temp, rs_wb_temp, de_mbdwb_temp, dlnes_mbdwb_temp, rsdwb_temp, foftk_wb_temp, fdwb_temp = QSat_2(
            wb_temp + C, Pressure)
    ...
    if 1:  # ConvergenceMode:
        ...
        while (len(subdo) > 0) and (iter <= maxiter):
            ...
            es_mb_wb_temp, rs_wb_temp, de_mbdwb_temp, dlnes_mbdwb_temp, rsdwb_temp, foftk_wb_temp, fdwb_temp = QSat_2(
                wb_temp[subdo] + C, Pressure[subdo])
            delta = 0 * wb_temp
        ...
        if any(map(len, subdo)):  # len(subdo)>0:
            ...
            for www in subdo[0]:
                #    print(www)
                print(
                    'WARNING-Wet_Bulb failed to converge. Setting to T: WB, P, T, RH, Delta: %0.2f, %0.2f, %0.1f, %0.2g, %0.1f' % (
                    Twb[www], Pressure[www], \
...
def get_wet_bulb(temp_c, press_mbar, humidity):
    """
    ...
    tempC = np.array([temp_c])
    Pres = np.array([press_mbar * 100.0])
    relHum = np.array([humidity])
                    
    Twb, Teq, epott = WetBulb(tempC, Pres, relHum)
    wet_bulb_temp = float(Twb[0].__str__())

    return round(float(wet_bulb_temp), 1)
...
if __name__ == "__main__":
    ...
    # print('Twb=' + Twb.__str__())
    # print('Teq=' + Teq.__str__())
    # print('epott=' + epott.__str__())

# ========

    wet_bulb_temp_c = get_wet_bulb(31, 1013.25, 70.0)
